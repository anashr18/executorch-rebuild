...
std::string JsonValue::dump(int indent) const {
  ...
  if (is_null()) {
    ...
    for (const auto& [key, value] : obj) {
      out << indent_string(indent + 2) << '"' << key << "\": " << value.dump(indent + 2);
      if (index + 1 < obj.size()) {
        out << ",";
      }
      out << "\n";
...
JsonParser::JsonParser(std::string source)
    : source_(std::move(source)), data_(source_.data()), size_(source_.size()), pos_(0) {}
...
JsonValue JsonParser::parse_value() {
  ...
  switch (ch) {
    ...
    case '"':
      return parse_string();
    case 't':
      return parse_literal("true", JsonValue::value_type{true});
    case 'f':
      return parse_literal("false", JsonValue::value_type{false});
    case 'n':
      return parse_literal("null", JsonValue::value_type{nullptr});
    default:
      if (ch == '-' || std::isdigit(static_cast<unsigned char>(ch))) {
        return parse_number();
      }
      raise(std::string("Unexpected character '") + ch + "' while parsing JSON value");
  }
}
...
JsonValue JsonParser::parse_number() {
  ...
  while (pos_ < size_ && std::isdigit(static_cast<unsigned char>(data_[pos_]))) {
    ++pos_;
  }
  if (consume_if('.')) {
    if (pos_ >= size_ || !std::isdigit(static_cast<unsigned char>(data_[pos_]))) {
...
[[noreturn]] void JsonParser::raise(const std::string& message) const {
  throw std::runtime_error("JSON parse error at position " + std::to_string(pos_) + ": " + message);
}
